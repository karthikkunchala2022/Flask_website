...
jwt_secret = os.getenv('JWT_SECRET', '3ea7100a7649e09a1b27b93a9320634db59c244d1bad746d')  # Replace with a secure key
...
def generate_token(msme_id):
    token = jwt.encode({'msme_id': msme_id, 'exp': datetime.utcnow() + timedelta(hours=96)}, jwt_secret, algorithm='HS256')
    return token
...
def register():
    ...
    try:
        ...
        token = generate_token(msme_id)
        print(f"Generated token link: http://yourdomain.com/api/dashboard?token={token}")
...
def api_dashboard():
    ...
    return jsonify({
        ...
        'total_amount_due': str(total_amount_due),
        ...
        'amount_due_today': str(amount_due_today),
        'total_vendors': total_vendors,
        'next_7_days_due_payments': next_7_days_due_payments
    })
@app.route('/api/upcoming_payments', methods=['GET'])
def api_upcoming_payments():
    ...
    try:
        ...
        upcoming_payments_list = [{'vendor_name': row[0], 'due_date': row[1].strftime('%Y-%m-%d'), 'amount': str(row[2])} for row in upcoming_payments]
    except Exception as e:
        return jsonify({'message': 'Error fetching data', 'error': str(e)}), 500
    finally:
        cursor.close()

    return jsonify(upcoming_payments_list)
